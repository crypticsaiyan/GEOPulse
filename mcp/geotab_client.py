...
class GeotabClient:
    ...
    def get_active_faults(self):
        """Get active fault codes from last 7 days. Cached 5min."""
        now = datetime.now(timezone.utc)
        from_date = (now - timedelta(days=7)).isoformat()

        params = {
            "typeName": "FaultData",
            "search": {"fromDate": from_date, "toDate": now.isoformat()},
            "resultsLimit": 100
        }
        ...
        for f in (faults or []):
            ...
            formatted.append({
                ...
                "source": f.get("source", {}).get("name", "Unknown") if isinstance(f.get("source"), dict) else str(f.get("source", "")),
                "failure_mode": f.get("failureMode", {}).get("name", "") if isinstance(f.get("failureMode"), dict) else "",
            })
        return formatted
