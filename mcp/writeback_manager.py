...
Methods:
    after_morning_analysis() → Groups for faults + welfare
...
class WritebackManager:
    ...
    def after_morning_analysis(self, analysis_results):
        ...
        Creates groups for critical faults and welfare checks.
        """
        actions = []
        today = date.today().isoformat()

        # Vehicles with critical faults
        faults = analysis_results.get("critical_faults", [])
        if faults:
            vehicle_ids = [f["device_id"] for f in faults if f.get("device_id")]
            if vehicle_ids:
                try:
                    group_id = self.geotab.create_group(
                        f"Needs Attention {today}", vehicle_ids
                    )
                    actions.append({"type": "group", "name": f"Needs Attention {today}", "id": group_id, "vehicles": len(vehicle_ids)})
                except Exception as e:
                    logger.error(f"Failed to create fault group: {e}")
